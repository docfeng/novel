...
操作:
  搜索:search
    获取搜索html getsearchhtml(name) return html
    搜索htmltoarr searchtoarr({name,html}) return [url,title,realurl]
    获取真实搜索结果 getsearchreal(arr) return [title,url]
    保存搜索结果 savesearch(name,arr[title,url])
    读取搜索结果 readsearch(name) return [title,url]
...
bds={
  get:function(str){
    ...
    return new Promise(function(resolve){
      get("http://www.baidu.com/s?q1="+str+
          "&q2=&q3=&q4=&rn=20&lm=0&ct=0"+
         "&ft=&q5=1&q6=&tn=baiduadv",
        function(html){
           resolve(bdstoa2({str:str,html:html}));
        });
    });
  },
  savesearch:function(str,arr){
    //保存搜索结果
  ...
  getlist:function(url){
    this.getlisthtml(url).then(function(json){
    	
    });
    
  },
  savelist:function(name,arr){
    //保存目录
    fso.fso.write("Shelf/"+name+".json",JSON.stringify(arr),false);
    //warn("保存目录");
  },
  saveShelf:function(name,arr,url,i){
    var i=i||0;
    var json={
		  	    name:name,
        url:url,
...
funcs={
  ...
  tfun2:function(obj){
      ...
      //如果本地没有结果;
      if(!arr[i][3]){
        bds.getlisthtml(arr[i][0]).then(function(json){
  ...
  showpage:function(title,url){
      bds.getpage(url).then(function(json){
...
getpage_async=function(url){
  return gethtml(url).then(function(json){
  	var html=json.html
  	return new Promise(function(resolve){
